...
Type *TypeTable::getCommonType(Type *T, Type *U) const {
    ...
    if (T == getIntType() && U == getStringType()) {
        return getVoidType();
    }
    if (T == getStringType() && U == getIntType()) {
        return getVoidType();
    }
    ...
    do {
        ...
        while (T != U2 && !isBuiltinType(U2)) {
            if (!getParentClass(U2->getClass())) {
                break;
            } else {
                U2 = getType(getParentClass(U2->getClass()));
            }
        }
        ...
        if (T == getVoidType() || isBuiltinType(T)) {
            return T;
        }
        if (T == getNullType()) {
            return U;
        }
        if (getParentClass(T->getClass())) {
            T = getType(getParentClass(T->getClass()));
        } else {
            T = nullptr;
        }
