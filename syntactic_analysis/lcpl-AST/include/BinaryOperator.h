...
public:
  /// \brief The kinds of binary operators in LCPL
  enum BinOpKind {
    Add,
    Sub,
    Mul,
  ...
  bool isArithmetic() const {
    return operatorKind >= FirstArithmetic &&
           operatorKind <= LastArithmetic;
  }

  bool isComparison() const {
    return operatorKind >= FirstComparison &&
           operatorKind <= LastComparison;
  }
...
typedef std::unique_ptr<BinaryOperator> BinaryOperatorPtr;
